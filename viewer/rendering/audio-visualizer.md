...
- 没有 tap、暂停、或 `prefers-reduced-motion: reduce`：只画一条静止基线
  （spectrum 在底部、waveform 在中线），**不**启动 `requestAnimationFrame`。
...
- `click`，或画布获得焦点后按 `Enter` / `Space`，按 `MODES` 的顺序切到下一个效果（当前 spectrum → waveform → spectrum）。
- `AudioVisualizerMode` 由 `MODES` 派生，新增效果时改这一处，类型和点击循环顺序会一起跟上。
- 切换后立刻补一帧：`analyser` 已存在时按新模式重设 `smoothingTimeConstant`（spectrum 用 `smoothing`，其余用 0），
...
| 曲线形状、平滑度、频率分布指数、高频截断比例、幅度、线宽 | `audio-visualizer.ts`（`trace*` 方法体或构造默认值） |
...
- 不做峰值保持、不做列内 min/max 聚合、不做曲线插值——保持单折线的轻量实现；
