export interface Insta360PhotoInspection {
  readonly kind: "photo";
  readonly width: 5952;
  readonly height: 2976;
...
function inspectExif(bytes: Uint8Array, start: number, end: number) {
  if (end - start < 14 || readAscii(bytes, start, 6) !== "Exif") return undefined;
  const tiff = start + 6;
  const order = readAscii(bytes, tiff, 2);
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    if ((tag !== 0x010f && tag !== 0x0110) || type !== 2 || valueCount === undefined || valueCount < 1 || valueCount > 256) continue;
    ...
    const value = readAscii(bytes, valueOffset, valueCount);
...
export function inspectInsta360Photo(bytes: Uint8Array): Insta360PhotoInspection | undefined {
  ...
  return width === 5952 && height === 2976 && make === "Arashi Vision" && model === "Insta360 X3"
    ? { kind: "photo", width, height, make, model }
    : undefined;
}
