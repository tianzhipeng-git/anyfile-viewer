...
export function inspectPhotoshopHeader(bytes: Uint8Array): PhotoshopHeader | undefined {
  ...
  if (String.fromCharCode(...bytes.subarray(0, 4)) !== "8BPS") return undefined;
  ...
  const maxDimension = version === 1 ? 30_000 : 300_000;
  if (channels < 1 || channels > 56 || width < 1 || height < 1 || width > maxDimension || height > maxDimension) return undefined;
  if (!VALID_DEPTHS.has(depth) || colorMode > 9) return undefined;
  return { version, width, height, channels, depth: depth as PhotoshopHeader["depth"], colorMode };
}
