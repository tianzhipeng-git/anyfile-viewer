export interface DjiOsmoPhotoInspection {
  readonly kind: "photo";
  readonly device: "Osmo 360";
  readonly width: 15520;
  readonly height: 7760;
}
...
export function inspectDjiOsmoPhoto(bytes: Uint8Array): DjiOsmoPhotoInspection | undefined {
  ...
  while (offset + 4 <= bytes.length) {
    ...
    if (SOF_MARKERS.has(marker) && payloadStart + 5 <= segmentEnd) {
      ...
      equirectangular ||= /GPano:ProjectionType=(?:"|')equirectangular(?:"|')/.test(xmp)
        || /<GPano:ProjectionType>equirectangular<\/GPano:ProjectionType>/.test(xmp);
      panoramaViewer ||= /GPano:UsePanoramaViewer=(?:"|')True(?:"|')/.test(xmp)
        || /<GPano:UsePanoramaViewer>True<\/GPano:UsePanoramaViewer>/.test(xmp);
    }
    offset = segmentEnd;
  }
  return width === 15520 && height === 7760 && make === "Osmo" && model === "OQ001" && equirectangular && panoramaViewer
    ? { kind: "photo", device: "Osmo 360", width, height }
    : undefined;
}
