...
function start() {
  ...
  return { controller, opening: createDwgWorkerClient(controller.signal, copy) };
}
...
it("uses R2 first and transfers input only after initialization", async () => {
  ...
  expect(first.postMessage.mock.calls[0][0]).toEqual({ type: "init", runtimeUrl: "https://assets.anyfile.top/vendor/libredwg/0.14-anyfile.1/dist/libredwg-web.js" });
  ...
  first.reply({ type: "opened", result: { batches: [] } });
  await expect(result).resolves.toEqual({ batches: [] });
...
it.each(["initialization", "worker"])("disposes failed R2 %s before trying same-origin", async (failure) => {
  ...
  expect(first.terminate).toHaveBeenCalledOnce();
  expect(second.postMessage.mock.calls[0][0]).toEqual({ type: "init", runtimeUrl: `${location.origin}/vendor/libredwg/0.14-anyfile.1/dist/libredwg-web.js` });
...
it("bounds stalled parsing and leaves no timers after disposal", async () => {
  vi.useFakeTimers(); const {opening} = start();
  await vi.advanceTimersByTimeAsync(0); TestWorker.instances[0].reply({type:"ready"});
  const client = await opening;
  const rejected = expect(client.open(new ArrayBuffer(128))).rejects.toMatchObject({code:"resource-limit"});
  await vi.advanceTimersByTimeAsync(30_000); await rejected; client.dispose();
  expect(TestWorker.instances).toHaveLength(1); expect(TestWorker.instances[0].terminate).toHaveBeenCalledOnce();
  expect(vi.getTimerCount()).toBe(0);
});
