...
vec4 sampleEac(vec3 direction) {
  ...
  float u = (local.x + face) * (1.0 - 4.0 / uTextureSize.x) / 3.0 + 2.0 / uTextureSize.x;
  float v = local.y * (1.0 - 4.0 / uTextureSize.y) + 2.0 / uTextureSize.y;
  return row < 0.5 ? texture2D(uTexture0, vec2(u, v)) : texture2D(uTexture1, vec2(u, v));
}
