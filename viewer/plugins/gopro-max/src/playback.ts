...
import type { GoProPanoramaRenderer } from "./panorama-renderer";
import { abortError } from "./read-blob";
import { formatTime, type GoProMaxViewerElements, type goProMaxUiCopy } from "./ui";
...
type UiCopy = ReturnType<typeof goProMaxUiCopy>;

async function inspectMedia(file: File, inspection: GoProMaxVideoInspection, signal: AbortSignal): Promise<GoProMaxMedia> {
  const input = new Input({ source: new BlobSource(file, { maxCacheSize: BLOB_CACHE_BYTES }), formats: [MP4] });
  const abort = () => input.dispose();
  signal.addEventListener("abort", abort, { once: true });
  try {
    if (signal.aborted) throw abortError();
    if (!await input.canRead() || await input.getFormat() !== MP4) throw new ViewerError("invalid-file", "Invalid GoPro MAX MP4 container.");
    ...
    if (dimensions[0] !== inspection.width || dimensions[1] !== inspection.height
      ...
      || videoCodecs.some((codec) => codec !== "hevc")) {
      throw new ViewerError("invalid-file", "Unexpected GoPro MAX media tracks.");
    }
    ...
    if (!Number.isFinite(start) || !Number.isFinite(audioStart) || !Number.isFinite(duration) || duration <= start || audioStart >= duration) {
      throw new ViewerError("invalid-file", "Invalid GoPro MAX track timeline.");
    }
...
export class GoProMaxPlayback {
  ...
  private constructor(
    media: GoProMaxMedia,
    inspection: GoProMaxVideoInspection,
    renderer: GoProPanoramaRenderer,
    elements: GoProMaxViewerElements,
    copy: UiCopy,
  ) {
    this.#media = media;
  ...
  static async open(
    ...
    const playback = new GoProMaxPlayback(media, inspection, renderer, elements, copy);
    try {
      ...
      throw new ViewerError("unsupported-environment", "The browser could not decode the first GoPro MAX frames.", { cause: error });
    }
  }

  private async initialize() {
    const frames = await Promise.all(this.#videoSinks.map((sink) => sink.getCanvas(this.#position)));
    if (!frames[0] || !frames[1]) throw new ViewerError("invalid-file", "The EAC tracks have no first frame.");
