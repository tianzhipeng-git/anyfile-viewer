...
vi.mock("./playback", () => ({ GoProMaxPlayback: { open: playbackMocks.open } }));

import { goProMaxViewer } from "./index";
...
describe("GoPro MAX viewer lifecycle", () => {
  it("renders and disposes an equirectangular MAX photo", async () => {
    const close = vi.fn();
    vi.stubGlobal("createImageBitmap", vi.fn().mockResolvedValue({ width: 5760, height: 2880, close }));
    const test = context(new File([goProMaxPhotoBytes()], "panorama.jpg"));
    const controller = await goProMaxViewer.open(test.context);
    expect(test.container.querySelector(".anyfile-gopro-max-viewer__canvas")).not.toBeNull();
  ...
  it("initializes and disposes dual-track video playback", async () => {
    const test = context(new File([goProMaxVideoBytes()], "capture.360"));
    const controller = await goProMaxViewer.open(test.context);
  ...
  it("rejects a lookalike file and leaves no DOM behind", async () => {
    const test = context(new File([goProMaxPhotoBytes("Other")], "panorama.jpg"));
    await expect(goProMaxViewer.open(test.context)).rejects.toMatchObject({ code: "invalid-file" });
    expect(test.container.children).toHaveLength(0);
  });
});
