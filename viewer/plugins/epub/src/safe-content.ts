...
import { EPUB_LIMITS, elements, localReference, parseXml, type Publication } from "./publication";
...
export async function prepareChapter(
  ...
  try {
    const source = parseXml(await zip.read(path, EPUB_LIMITS.chapter, signal));
    ...
    target.body.dir = body.getAttribute("dir") === "rtl" ? "rtl" : book.direction;
