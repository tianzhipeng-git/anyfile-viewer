...
- 状态：分阶段实施；`viewer-rendering` 基础层已落地，Lit UI 仍未采用
- 范围：`viewer/` 下的查看器插件及其共享依赖
- 目标读者：查看器协议、插件和网站外壳的维护者

每次你(AI Agent、编程助手等)读到这个提案，都要评估两个事:
- 这个提案要不要进行修改，以满足当前现状和未来规划？
- 这个提案要不要现在就落地, 已经到了需要落地的时候？

阶段 3 图片实施已经满足第二个 Canvas 调用方门槛，并落地 `InteractiveViewport`、`CanvasSurface` 与 `ResourceScope`。现有 `viewer-ui` 仍保持原生 DOM；本次没有出现足以引入 Lit 的复杂共享组件状态。
