...
实现结果：LibRaw 生命周期、元数据、缩略图/显影与资源上限已提取到内部 `@anyfile/raw-decoder` 共享包；Insta360 probe 先读取 TIFF 头中的 IFD0 偏移，再定点读取不超过 64 KiB 的目录窗口，严格校验 DNGVersion、Arashi Vision / Insta360 X3 和 2976×5952 布局。`open()` 会通过 LibRaw 元数据再次复验，基础显影后按 TB 布局切成两张 2976×2976 镜头纹理。其他 DNG 的 Insta360 probe 返回 0，继续由 camera-raw 插件处理。

真实样本验收（2026-09-02）：macOS 15.6.1 / Codex 内置 Chromium 浏览器，使用 `IMG_20230815_194449_00_797.dng`（约 34 MiB）。已验证专用查看器路由、LibRaw 基础显影、上下镜头映射、WebGL 首屏、全景交互 UI 与资源清理；文件未离开本地浏览器。
...
### 2026-09-08：双轨 HEVC 软件解码回退

浏览器首帧解码不可用时，现代双轨 INSV 先尝试共享 FFmpeg 的独立全景模式。两路 3840² HEVC + AAC 在单 Worker 内分段读取、解码；每路输出 1920² I420，保留逐文件标定和全景交互。播放层在解码不足时共同暂停画面、音频和播放时钟，不以持续音画失步换取进度条前进。运行环境缺失时保留静态预览，文件错误或资源超限不隐藏。性能和验收见 [INSV FFmpeg 播放](../videos/insv-ffmpeg-playback.md)。
