...
- 使用文件选择器一次选择两个文件时，宿主把它们显示在文件树中，但不会创建 `WorkspaceReader`；
- 因此插件无法从当前 `.insv` 访问同批选择的另一文件。

建议在不修改公共协议的前提下，为多选文件构造内存只读工作区：
...
文件名冲突时不能静默选择其中一个，应拒绝构造有歧义的工作区或向用户显示明确错误。
...
完成结果：可以同时选择两段 `.insv` 或打开所在文件夹，并以高清全景方式播放。
