...
1. 宿主按文件名和扩展名产生候选；
2. 候选按需加载轻量 `probe()`；
3. probe 有界读取容器头、轨道描述和 codec 配置，并结合当前浏览器的能力信号返回 0–5；
4. 宿主按动态等级和注册顺序稳定排序；
5. 只加载默认或用户选中的完整插件；
6. `open(context)` 创建 Object URL 并让真实媒体元素加载当前文件；
7. 切换时先 abort，再幂等 dispose 并清理容器。

`canPlayType()`、Media Capabilities 或未来 `VideoDecoder.isConfigSupported()` 都只能作为能力信号。它们不能证明当前容器索引、所有轨道、真实样本和 seek 可用，因此不能取代 `open()`。
...
打开成功不能只等待 `loadedmetadata`：它可能证明容器元数据可读，却不能证明首个视频样本可解码。具体完成事件和超时策略必须在阶段 1 spike 中用真实浏览器验证，不能拍脑袋设置固定延迟。
