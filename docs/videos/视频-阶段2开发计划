...
- AVI、MPEG-PS、MPEG-1/2 Video、AC-3/DTS 的依赖 spike 已闭环为后续按需求项目：Mediabunny 不提供对应 demux/decoder；libav.js 可行但 npm 完整包约 167 MB，需先产出受控裁剪 variant、来源与许可交付方案，不能作为本阶段万能 fallback。它们不是阶段 2 已声明范围，也不阻塞本阶段完成。
...
- Matroska 管线稳定后，新增格式主要是增加 container adapter、probe 识别和组合证据，不复制播放器：
  ...
  4. AVI、MPEG-PS：Mediabunny 当前不直接覆盖，先做依赖 spike，再决定新增 demuxer 或受控 FFmpeg/WASM；
  5. AC-3、DTS、Theora、MPEG-2 等 Chromium 原生 decoder 缺口，按使用价值增加独立延迟加载的 WASM provider。
- 新增 codec provider不得进入其他 codec 的加载路径；例如打开 MKV/AVC/AAC 时不能下载 AC-3 或 MPEG-2 decoder。
- ProRes、DNx、MXF、timecode 等仍留给阶段 3 `professional-video`，不塞进普通播放器。
...
- 不自行构建通用 FFmpeg/WASM；OGV.js 使用锁定的上游预构建 Ogg 专用资产。其余缺口先形成可审计 adapter/provider 决策，不加入万能 fallback。
