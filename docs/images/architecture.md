...
Probe 可以读取必要的 magic bytes、容器结构、内部 codec 或子类型信息，但只负责排序。`open()` 仍要进行适合当前实现路径的校验、资源策略和真正渲染。默认插件打开失败时展示错误，不自动尝试下一项。
...
每个插件在完整解码前尽可能有界读取并校验文件头。资源策略按实现路径决定：

- 浏览器原生 `<img>` 等路径应避免 JavaScript 完整文件副本和像素副本，实际解码容量交给浏览器，不要求设置缺少依据的统一文件、像素或帧数硬阈值；
- 自行解析或分配资源的 JS/WASM/Worker/Canvas/GPU 路径，应根据真实分配模型限制样本、帧、页、图层、tile、mip、元数据、容器嵌套、并发和缓存；
- 所有由插件执行的尺寸计算和内存乘法必须在分配前检查溢出。

达到插件明确声明的边界时返回 `resource-limit`，损坏或伪装格式返回 `invalid-file`。压缩文件大小不能单独代表解码成本，也不应作为所有实现路径的统一替代指标。
