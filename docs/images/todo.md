...
> 逐步覆盖已知且有实际价值的图像格式；为每种格式明确支持等级、功能边界、资源限制和已知缺失；保持新增格式不会扩大网站首包，也不会迫使无关插件加载重型依赖。
...
- [ ] 为第一阶段格式收集可再分发的正常、损坏、截断和极端尺寸样例。
- [ ] 填写支持矩阵中的首批基线数据。
- [ ] 明确单文件大小、总像素、帧数、解码后内存和元数据块上限。
- [ ] 对浏览器原生路径验证动画、EXIF orientation、ICC、alpha 和取消行为。
- [ ] 为首批图片候选实现轻量 probe，并验证同扩展名候选按动态支持等级稳定排序。
- [ ] 验证 probe 不进入 `/view` 首包、不加载完整 renderer，并能在文件切换时取消。
- [ ] 通过第一款真实插件反推最小图片查看 UI，不提前建立通用 decoder 协议。
