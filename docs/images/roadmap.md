...
- 新增独立 `photoshop-document` 插件，当前只声明 `.psd`；有界 probe 明确拒绝 PSB version 2，避免把依赖尚不支持的 `.psb` 误报为可查看。
- `ag-psd@31.0.2` 在专用 Worker 中读取 PSD 结构并解码保存的合成图，跳过图层像素、缩略图和链接文件；当前支持等级为 3，不把扁平预览描述成图层级完整查看。
...
- 自动测试与生产首包门禁已通过；复杂真实 PSD 与跨尺寸手工 smoke 尚待补充，因此验证状态为 `implemented`。

### 图层文档

- PSD：已完成合成预览与图层数量；下一步是图层列表，再评估分层合成；
- PSB：等待可验证且资源边界可控的解码路径；
