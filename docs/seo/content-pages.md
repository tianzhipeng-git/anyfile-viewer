...
旧版 `src/lib/catalog.ts` 与插件 Manifest 分别手写格式能力时曾存在漂移，例如 catalog 列出 OBJ、glTF、STL 和 FIG，但插件注册表没有对应专用查看器。这些页面不能继续以“受支持格式”身份发布，否则会产生可索引但无法兑现的落地页。PSD 现由专用插件提供等级 3 合成预览，AI 则只在包含 PDF 兼容表示时由 PDF.js 提供等级 4 查看；结构化支持资料必须保留这些边界。
