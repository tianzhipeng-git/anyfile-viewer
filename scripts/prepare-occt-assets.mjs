...
const sourceRoot = join(projectRoot, "third_party/occt-import-js", version);
const targetRoot = join(projectRoot, "public/vendor/occt-import-js", version);
const buildInfo = JSON.parse(await readFile(join(sourceRoot, "build-info.json"), "utf8"));
if (buildInfo.artifactVersion !== version) throw new Error(`Unexpected OCCT artifact version: ${buildInfo.artifactVersion}`);
...
for (const [fileName, expected] of Object.entries(buildInfo.artifacts)) {
  ...
  if (bytes.byteLength !== expected.bytes || sha256 !== expected.sha256) {
    throw new Error(`OCCT artifact integrity check failed: ${fileName}`);
  }
