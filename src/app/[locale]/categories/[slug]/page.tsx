...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  return localizedPageMetadata({ locale, path, title: category.searchTitle, description: category.description });
}
...
export default async function CategoryPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  return <>
    ...
    <section className="bg-background py-14 sm:py-20"><div className="content-shell flex flex-col gap-10">
      ...
      <div className="max-w-4xl"><p className="mb-4 text-sm font-semibold text-primary">{category.eyebrow}</p><h1 className="display-title text-5xl sm:text-6xl">{category.title}</h1><p className="mt-6 text-xl leading-8 text-muted-foreground">{category.description}</p></div>
    </div></section>
