...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; pluginId: string }> }): Promise<Metadata> {
  ...
  return localizedPageMetadata({ locale, path, title: plugin.title, description: plugin.description });
}
...
export default async function PluginPage({ params }: { params: Promise<{ locale: string; pluginId: string }> }) {
  ...
  return <>
    ...
    <section className="bg-background py-14 sm:py-20"><div className="content-shell flex flex-col gap-10">
      ...
      <div className="max-w-4xl"><div data-nosnippet className="mb-5 flex flex-wrap gap-2"><Badge variant="secondary">{plugin.pluginId}</Badge><Badge variant="outline">protocol v{plugin.manifest.protocolVersion}</Badge><Badge variant="outline">workspace: {plugin.manifest.workspaceAccess}</Badge></div><h1 className="display-title text-5xl leading-none sm:text-6xl">{plugin.title}</h1><p className="mt-6 text-xl leading-8 text-muted-foreground">{plugin.description}</p>{panoramaViewer && <Link className="mt-6 inline-flex items-center gap-2 font-semibold text-primary" href={localePath(locale, `/viewers/${panoramaViewer.viewerId}`)}>{locale === "zh-CN" ? "查看面向用户的相机支持页" : "See the camera viewer guide"}<ArrowRightIcon className="size-4" /></Link>}</div>
    </div></section>
    <section className="bg-foreground py-16 text-background"><div className="content-shell grid gap-12 lg:grid-cols-2">
      <div><NetworkIcon className="mb-4 size-7 text-primary" /><h2 className="display-title mb-6 text-3xl">{locale === "zh-CN" ? "查看流程" : "Viewing architecture"}</h2><div data-nosnippet className="mb-6 leading-7 opacity-80">{plugin.summary}</div><ol className="space-y-5">{plugin.architecture.map((item, index) => <li className="flex gap-4" key={item}><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-background/10 text-sm">{index + 1}</span><span className="leading-7 opacity-80">{item}</span></li>)}</ol></div>
