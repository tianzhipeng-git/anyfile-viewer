...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; extension: string }> }): Promise<Metadata> {
  ...
  return localizedPageMetadata({ locale, path, title: format.searchTitle, description: format.description });
}
  ...
  return <>
    ...
    <section className="bg-background py-14 sm:py-20"><div className="content-shell flex flex-col gap-10">
      ...
      <div className="grid gap-10 lg:grid-cols-[1fr_0.55fr] lg:items-end"><div className="flex max-w-3xl flex-col items-start gap-6">
        ...
        <p className="text-xl leading-8 text-muted-foreground">{format.description}</p>
      ...
      </div><div className="rounded-2xl border bg-muted p-6"><LockKeyholeIcon className="mb-4 size-7 text-primary" aria-hidden="true" /><h2 className="mb-2 text-lg font-semibold">{locale === "zh-CN" ? "文件留在当前设备" : "Your file stays on this device"}</h2><p className="leading-7 text-muted-foreground">{locale === "zh-CN" ? "选择文件即可在浏览器中查看，无需注册或安装软件。文件始终留在你的设备上，不会上传。" : "Choose a file to view it in your browser. No signup or software installation needed. Your file stays on your device and is never uploaded."}</p></div></div>
    </div></section>
    ...
    <section className="bg-background py-16 sm:py-20"><div className="content-shell grid gap-12 lg:grid-cols-[1fr_0.7fr]">
      <div data-nosnippet><p className="mb-2 text-sm font-semibold text-primary">{locale === "zh-CN" ? "实现透明度" : "IMPLEMENTATION"}</p><h2 className="display-title mb-6 text-3xl">{locale === "zh-CN" ? "负责此格式的查看器" : "Viewers responsible for this format"}</h2><p className="mb-6 leading-7 text-muted-foreground">{format.introduction}</p><div className="grid gap-4 sm:grid-cols-2">{plugins.map((plugin) => <Card key={plugin.pluginId}><CardHeader><CardTitle>{plugin.manifest.name[locale] ?? plugin.manifest.name.en}</CardTitle><CardDescription>{plugin.description}</CardDescription><Link className="mt-3 inline-flex items-center gap-2 text-sm font-semibold text-primary" href={localePath(locale, `/plugins/${plugin.pluginId}`)}>{locale === "zh-CN" ? "查看实现与开源依赖" : "See implementation and open-source credits"}<ArrowRightIcon className="size-4" /></Link></CardHeader></Card>)}</div></div>
